  ...
  stages {
    ...
    stage('docker build & push image on build docker/build server') {
      ...
      steps {
        ...
        script {
          def REMOVE_FLAG = sh(returnStdout: true, script: "docker container ls -aq --filter name=.*smc-eureka-ctn.*") != ""
          echo "REMOVE_FLAG: ${REMOVE_FLAG}"
          if(REMOVE_FLAG){
            // sh 'docker container ls -aq --format {{.ID}} --filter name=smc-eureka-ctn | xargs docker container rm -f'
            // sh 'docker container rm -f $(docker container ls -aq --format {{.ID}} --filter name=.*smc-eureka-ctn.*)'
            sh 'docker container rm -f $(docker container ls -aq --filter name=.*smc-eureka-ctn.*)'
          }
        }

        // docker rmi old images before build: remove only there are images found
        script {
          def REMOVE_FLAG = sh(returnStdout: true, script: "docker image ls -q *${DOCKHUB_USERNAME}/smc-eureka-srv*") != ""
          echo "REMOVE_FLAG: ${REMOVE_FLAG}"
          if(REMOVE_FLAG){
            // sh 'docker image ls --format {{.ID}} *${DOCKHUB_USERNAME}/smc-eureka-srv* | xargs docker image rm -f'
            // sh 'docker image rm -f $(docker image ls --format {{.ID}} *${DOCKHUB_USERNAME}/smc-eureka-srv*)'
            sh 'docker image rm -f $(docker image ls -q *${DOCKHUB_USERNAME}/smc-eureka-srv*)'
          }
        }
        ...
        // script {
          ...
          //   def customImage = docker.build("${DOCKHUB_USERNAME}/smc-eureka-srv:latest", '-f Dockerfile .')
        ...
        withCredentials([usernamePassword(credentialsId: "DockerHub_${DOCKHUB_USERNAME}", usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
          ...
          sh 'docker image build -t ${DOCKHUB_USERNAME}/smc-eureka-srv:latest .'
          sh 'docker push ${DOCKHUB_USERNAME}/smc-eureka-srv:latest'
        }
      }
    }
    ...
    stage('docker pull image and docker run image on docker/deploy server') {
      ...
      steps {
        ...
        // sh 'docker pull ${DOCKHUB_USERNAME}/smc-eureka-srv'

        // docker run images
        sh 'docker run -d -p 8761:8761 -v smc-data:/smc-data --network host --name smc-eureka-ctn ${DOCKHUB_USERNAME}/smc-eureka-srv -Xms128m -Xmx128m'
      }
    }
